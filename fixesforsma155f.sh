...
#cd ../out/target/product/a15/obj/KERNEL_OBJ/kernel-5.10/arch/arm64/boot/
#~/mkbootimg/mkbootimg.py --header_version 4 --kernel Image.gz --output boot.img --ramdisk ~/archives/out/ramdisk --os_version 12.0.0 --os_patch_level "2024_03"
#~/sma155f/kernel/prebuilts/kernel-build-tools/linux-x86/bin/avbtool add_hash_footer --partition_name boot --partition_size $((64 * 1024 * 1024)) --image boot.img --algorithm SHA256_RSA4096 --key ~/private_key.pem
#cp ~/sma155f/out/target/product/a15/obj/KERNEL_OBJ/kernel-5.10/arch/arm64/boot/boot.img "$STARTPATH/"
#cp ~/sma155f/out/target/product/a15/obj/KERNEL_OBJ/kernel-5.10/arch/arm64/boot/Image "~/archives/maggi"
#cd ~/archives/maggi
#mv Image kernel
#echo "copied Image for magiskboot"
#~/magisktools/magiskboot repack ~/archives/boot.img boot.img
#~/magisktools/magiskboot sign boot.img ~/privatekey.pk8
#cp boot.img "$STARTPATH/"
#echo "copied magiskboot packed and signed boot.img to startpath:"
#echo $STARTPATH